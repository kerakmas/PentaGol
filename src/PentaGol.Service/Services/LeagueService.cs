...
public class LeagueService : ILeagueService
{
    ...
    public async ValueTask<LeagueResultDto> AddAsync(LeagueCreationDto dto)
    {
       var existLeague = await this.repository.SelectAsync(x => x.Name == dto.Name && x.IsDeleted == true);
        ...
        return this.mapper.Map<LeagueResultDto>(mapped); 
    }

    public async ValueTask<bool> RemoveAsync(long id)
    {
        var league = await this.repository.SelectAsync(x => x.Id == id && x.IsDeleted == false);
        if (league is null)
    ...
    {
        var leauges = await this.repository.SelectAll(x => x.IsDeleted == false).ToPagedList(@params).ToListAsync();
        return this.mapper.Map<IEnumerable<LeagueResultDto>>(leauges);
    }

    public async ValueTask<LeagueResultDto> RetrieveById(long id)
    {
        var leaugue = await this.repository.SelectAsync(l => l.Id == id && l.IsDeleted == false );
        if (leaugue is null)
            throw new PentaGolException(404, "Not Found");
    ...
    {
        var leauge = await this.repository.SelectAsync(l => l.Id == id && l.IsDeleted == false );
        if (leauge is null)
            throw new PentaGolException(404, "Not Found");
